  ...
  try {
    ...
    for (const file of jsFiles) {
      ...
      try {
        ...
        const trainDataMatch = fileContent.match(
          /export\s+const\s+trainData\s*=\s*({[\s\S]*?});/,
        );
        if (!trainDataMatch) {
        ...
        const forwardTrainNumber =
          trainData.forward?.train_number?.toString() || "";
        const reversePath = trainData.reverse?.path || "";
        const reverseTrainNumber =
          trainData.reverse?.train_number?.toString() || "";
