/**
 * DEPRECATED: This script is deprecated and will be removed in the future.
 *
 */
...
async function fetchTrainSchedule(fromCity, toCity, journeyDate) {
  try {
    ...
    console.error(
      `Error fetching schedule from ${fromCity} to ${toCity}:`,
      error.message,
    );
    return null;
  }
...
async function fetchTrainOffDays(trainModel, journeyDate) {
  try {
    ...
    const offDays = allDays.filter(
      (day) => !operatingDays.includes(day.substring(0, 3)),
    );
    ...
    console.error(
      `Error fetching off days for model ${trainModel}:`,
      error.message,
    );
...
async function main() {
  ...
  for (const route of paths) {
    ...
    for (const { date, label } of journeyDates) {
      console.log(
        `  📅 Fetching schedule for ${label} (${formatDate(date)})...`,
      );
    ...
    if (allSchedules.length > 0) {
      ...
      allSchedules.sort((a, b) => {
        ...
        return (
          getMinutes(a.departure_date_time) - getMinutes(b.departure_date_time)
        );
      });
